...
func MapConfigValues(cfg config.Config) []ConfigValue {
	return []ConfigValue{
		...
		{ID: "Mattermost.Enable", Type: ConfigTypeBoolean, Description: "Enables Mattermost notifications.", Value: fmt.Sprintf("%t", cfg.Mattermost.Enable)},
		{ID: "Mattermost.InteractiveMessages", Type: ConfigTypeBoolean, Description: "Enable interactive messages (e.g. buttons).", Value: fmt.Sprintf("%t", cfg.Mattermost.InteractiveMessages)},
...
func ApplyConfigValues(cfg config.Config, vals []ConfigValueInput) (config.Config, error) {
	...
	for _, v := range vals {
		...
		case "Mattermost.Enable":
			val, err := parseBool(v.ID, v.Value)
			if err != nil {
				return cfg, err
			}
			cfg.Mattermost.Enable = val
		case "Mattermost.InteractiveMessages":
			val, err := parseBool(v.ID, v.Value)
			if err != nil {
				return cfg, err
			}
			cfg.Mattermost.InteractiveMessages = val
		case "Twilio.Enable":
			val, err := parseBool(v.ID, v.Value)
			if err != nil {
				return cfg, err
