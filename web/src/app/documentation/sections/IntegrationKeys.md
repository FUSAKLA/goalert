...
| `severity` | _optional_   | Alert severity level: `info` (default), `warning`, `high`, or `critical`.                                                                                           |
...
curl -XPOST https://<example.goalert.me>/api/v2/generic/incoming?token=key-here&summary=test&severity=critical
```
